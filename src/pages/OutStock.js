...
function OutStock() {
    ...
    const totalCols = 7 + ((hasPermission('OutStock', 'update') || hasPermission('OutStock', 'delete')) ? 1 : 0);
    ...
    const fetchOutStocks = async (search = '', page = 1, limit = 10, filterParams = {}) => {
        try {
            ...
            const result = await fetchApi(`/outstock?${queryParams.toString()}`);

            let records = [];
            if (result.outStocks && Array.isArray(result.outStocks)) {
                records = result.outStocks;
            ...
            const mappedData = records.map(item => ({
                ...
                date: item.outQuantityDate ? item.outQuantityDate.split('T')[0] : (item.date ? item.date.split('T')[0] : '-'),
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            fetchOutStocks(searchQuery, currentPage, itemsPerPage, activeFilters);
        }, 200);
    ...
    const columnLabels = {
        ...
        date: 'Out Quantity Date',
        outQty: 'Out Quantity',
        outPrice: 'Out Price (Incl. GST)',
    ...
    const sortedData = useMemo(() => {
        ...
        if (sortConfig.key !== null) {
            sortableItems.sort((a, b) => {
                ...
                if (sortConfig.key === 'outQty' || sortConfig.key === 'outPrice' || sortConfig.key === 'total') {
    ...
    const handlePrint = async () => {
        try {
            ...
            const allData = await fetchApi(`/outstock?${queryParams.toString()}`);
            const stocksToPrint = Array.isArray(allData) ? allData : (allData.outStocks || []);

            const printWindow = window.open('', '', 'height=600,width=800');
            printWindow.document.write('<html><head><title>Out Stock List</title>');
            ...
            printWindow.document.write('<h1>Out Stock List</h1>');
            ...
            if (exportColumns.date) printWindow.document.write('<th>Out Quantity Date</th>');
            if (exportColumns.outQty) printWindow.document.write('<th>Out Quantity</th>');
            if (exportColumns.outPrice) printWindow.document.write('<th>Out Price (Incl. GST)</th>');
            ...
            stocksToPrint.forEach((item, index) => {
                const date = item.outQuantityDate ? item.outQuantityDate.split('T')[0] : (item.date ? item.date.split('T')[0] : '-');
                ...
                if (exportColumns.outQty) printWindow.document.write(`<td>${item.outQuantity || ''}</td>`);
                if (exportColumns.outPrice) printWindow.document.write(`<td>${(parseFloat(item.outPrice) * 1.18 || 0).toFixed(2)}</td>`);
    ...
    const handleExcelDownload = async () => {
        try {
            ...
            const allData = await fetchApi(`/outstock?${queryParams.toString()}`);
            const stocksToExport = Array.isArray(allData) ? allData : (allData.outStocks || []);
            ...
            const worksheetData = stocksToExport.map((item, index) => {
                ...
                if (exportColumns.date) row['Out Quantity Date'] = date;
                if (exportColumns.outQty) row['Out Quantity'] = item.outQuantity;
                if (exportColumns.outPrice) row['Out Price (Incl. GST)'] = (parseFloat(item.outPrice) * 1.18 || 0).toFixed(2);
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Out Stock');
            XLSX.writeFile(wb, 'OutStock_Report.xlsx');
        } catch (error) {
    ...
    const handleEdit = (item) => {
        navigate(`${getRolePrefix()}/out-stock/remove`, { state: { edit: item } });
    };

    const handleDelete = (id) => {
        Swal.fire({
            title: 'Are you sure?',
            text: "This will restore the stock to the product!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Yes, delete & restore!'
        }).then(async (result) => {
            if (result.isConfirmed) {
                try {
                    await fetchApi(`/outstock/${id}`, { method: 'DELETE' });
                    setData(prev => prev.filter(item => item.id !== id));
                    Swal.fire('Deleted!', 'Record deleted and stock restored.', 'success');
                } catch (error) {
    ...
    return (
        <div className="list-page-container">
            <div className="list-page-header">
                <h1 className="list-page-title">Out Stock</h1>
                <div className="list-page-action-buttons">
                    {hasPermission('OutStock', 'report') && (
                        <>
                    ...
                    {hasPermission('OutStock', 'add') && (
                        <button className="list-page-btn list-page-btn-add" onClick={() => navigate(`${getRolePrefix()}/out-stock/remove`)}>
                            - STOCK
                        </button>
                    )}
                </div>
            ...
            <div className="list-page-toolbar">
                ...
                <div className="list-page-filter-box">
                    ...
                    <button className="list-page-btn list-page-btn-submit" onClick={handleSubmit}>Submit</button>
                    <button className="list-page-btn list-page-btn-reset" onClick={handleReset}>Reset</button>
                </div>
            </div>
                ...
                <table className="list-page-table">
                    <thead>
                        <tr>
                            ...
                            <th onClick={() => requestSort('invoiceNo')}>INVOICE NO {sortConfig.key === 'invoiceNo' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}</th>
                            <th onClick={() => requestSort('date')}>OUT QTY DATE {sortConfig.key === 'date' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}</th>
                            <th onClick={() => requestSort('outQty')}>OUT QTY {sortConfig.key === 'outQty' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}</th>
                            <th onClick={() => requestSort('outPrice')}>OUT PRICE {sortConfig.key === 'outPrice' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}</th>
                            <th onClick={() => requestSort('total')}>TOTAL AMOUNT {sortConfig.key === 'total' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}</th>
                            {(hasPermission('OutStock', 'update') || hasPermission('OutStock', 'delete')) && <th>ACTION</th>}
                        </tr>
                    </thead>
                    <tbody>
                        {sortedData.length > 0 ? (
                            sortedData.map((item, index) => (
                                <tr key={item.id}>
                                    ...
                                    <td>{item.date}</td>
                                    <td>{item.outQty}</td>
                                    <td>{(parseFloat(item.outPrice) * 1.18 || 0).toFixed(2)}</td>
                                    <td>{(parseFloat(item.total) * 1.18 || 0).toFixed(2)}</td>
                                    {(hasPermission('OutStock', 'update') || hasPermission('OutStock', 'delete')) && (
                                        <td>
                                            <div className="list-page-action-icons">
                                                {hasPermission('OutStock', 'update') && (
                                                    <button className="list-page-icon-btn" title="Edit" onClick={() => handleEdit(item)}>
                                                ...
                                                {hasPermission('OutStock', 'delete') && (
                                                    <button className="list-page-icon-btn" title="Delete" onClick={() => handleDelete(item.id)}>
                            ...
                            <tr><td colSpan={totalCols} className="list-page-table-empty">No records found</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            <div className="list-page-footer">
                <div className="list-page-entries">
                    <span>Show</span>
                    <select className="list-page-entries-select" value={itemsPerPage} onChange={handleItemsPerPageChange}>
                ...
                <div className="list-page-info">Showing {totalEntries === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, totalEntries)} of {totalEntries} entries</div>
                <div className="list-page-pagination">
                    <button className="list-page-page-btn" disabled={currentPage === 1} onClick={() => paginate(currentPage - 1)}>Previous</button>
                    {pages.map(p => (
                        <button key={p} className={`list-page-page-num ${currentPage === p ? 'list-page-page-num-active' : ''}`} onClick={() => paginate(p)}>{p}</button>
                    ))}
                    <button className="list-page-page-btn" disabled={currentPage === totalPages || totalPages === 0} onClick={() => paginate(currentPage + 1)}>Next</button>
                </div>
            </div>
