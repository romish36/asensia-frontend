...
function ColorList({ onAdd, onEdit }) {
    ...
    const fetchColors = async (search = '', page = 1, limit = 10) => {
        try {
            setLoading(true);
            const result = await fetchApi(`/color?search=${search}&page=${page}&limit=${limit}`);
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            fetchColors(searchQuery, currentPage, itemsPerPage);
        }, 200);
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await fetchApi(`/color/${id}`, { method: 'DELETE' });
    ...
    // if (loading) return <div className="list-page-container">Loading colors...</div>;


    return (
        <div className="list-page-container">
            <div className="list-page-header">
                <h1 className="list-page-title">Color List</h1>
                <div className="list-page-action-buttons">
                    {hasPermission('Color', 'add') && (
                        <button className="list-page-btn list-page-btn-add" onClick={handleAdd}>ADD COLOR</button>
                    )}
                </div>
            </div>
            ...
            <div className="list-page-table-wrapper">
                <table className="list-page-table">
                    <thead><tr><th>COLOR NAME</th><th>ACTIONS</th></tr></thead>
                    <tbody>
                        {loading ? (
