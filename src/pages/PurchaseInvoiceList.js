...
function PurchaseInvoiceList({ onPreview, onAddInvoice, onEditInvoice, onNavigateToProfile }) {
  ...
  const columnLabels = {
    ...
    buyerTradeName: 'Company Name',
    companyState: 'Place of Supply',
  ...
  const totalCols = 5 + (isSuperAdmin ? 1 : 0) + (hasPermission('Purchase Order', 'view') ? 1 : 0) + ((hasPermission('Purchase Order', 'update') || hasPermission('Purchase Order', 'delete')) ? 1 : 0);
  ...
  const fetchSellers = async () => {
    try {
      const data = await fetchApi('/seller');
      setSellers(data);
    } catch (error) {
  ...
  const fetchPurchaseInvoices = async (searchQuery = '', currentPage = 1, limit = 10, filters = {}) => {
    try {
      ...
      const result = await fetchApi(`/purchase-order?${queryParams.toString()}`);
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      fetchPurchaseInvoices(search, page, pageSize, {
  ...
  }, [search, page, pageSize, appliedSeller, appliedStartDate, appliedEndDate]);
    ...
    if (sortConfig.key !== null) {
      sortableItems.sort((a, b) => {
        ...
        if (sortConfig.key === 'totalAmount') {
          aValue = typeof aValue === 'string' ? parseFloat(aValue.replace(/[^\d.-]/g, '')) : (aValue || 0);
          bValue = typeof bValue === 'string' ? parseFloat(bValue.replace(/[^\d.-]/g, '')) : (bValue || 0);
  ...
  const handleExcelDownload = async () => {
    try {
      ...
      if (appliedSeller) queryParams.append('seller', appliedSeller);
      ...
      if (result.purchaseOrders && Array.isArray(result.purchaseOrders)) {
        invoicesToExport = result.purchaseOrders;
      } else if (Array.isArray(result)) {
      ...
      const worksheetData = invoicesToExport.map((r, index) => {
        ...
        if (exportColumns.companyName) row['Seller Company Name'] = r.companyName;
        if (isSuperAdmin && exportColumns.buyerTradeName) row['Company Name'] = r.buyerTradeName || '';
        if (exportColumns.companyState) row['Place of Supply'] = r.companyState;
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'PurchaseInvoices');
      XLSX.writeFile(workbook, 'PurchaseInvoiceList.xlsx');
    } catch (error) {
  ...
  const handlePrint = async () => {
    try {
      ...
      let invoicesToPrint = [];
      if (result.purchaseOrders && Array.isArray(result.purchaseOrders)) {
        invoicesToPrint = result.purchaseOrders;
      } else if (Array.isArray(result)) {
        invoicesToPrint = result;
      }

      const printWindow = window.open('', '', 'height=600,width=800');
      printWindow.document.write('<html><head><title>Purchase Invoice List</title>');
      ...
      printWindow.document.write('<h1>Purchase Invoice List</h1>');
      printWindow.document.write('<table><thead><tr>');
      printWindow.document.write('<th>No.</th>');
      if (exportColumns.companyName) printWindow.document.write('<th>Seller Company Name</th>');
      if (isSuperAdmin && exportColumns.buyerTradeName) printWindow.document.write('<th>Company Name</th>');
      if (exportColumns.companyState) printWindow.document.write('<th>Place of Supply</th>');
      ...
      invoicesToPrint.forEach((r, index) => {
        ...
        if (exportColumns.companyName) printWindow.document.write(`<td>${r.companyName || ''}</td>`);
        if (isSuperAdmin && exportColumns.buyerTradeName) {
          printWindow.document.write(`<td>${r.buyerTradeName || ''}</td>`);
        }
        if (exportColumns.companyState) printWindow.document.write(`<td>${r.companyState || ''}</td>`);
  ...
  const deleteRow = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await fetchApi(`/purchase-order/${id}`, { method: 'DELETE' });
  ...
  const handleSellerClick = async (invoice) => {
    try {
      ...
      const sellerData = await fetchApi(`/seller/by-seller-id/${invoice.purchaseCompanyId}`);
      if (sellerData) {
        if (onNavigateToProfile) {
          onNavigateToProfile(sellerData);
        }
      } else {
  ...
  return (
    <div className="list-page-container">
      ...
      <div className="list-page-header">
        <h1 className="list-page-title">Purchase Invoice List</h1>
      ...
      <div className="list-page-toolbar">
        ...
        <div className="list-page-filter-box">
          ...
          <div className="list-page-filter-group">
            <label className="list-page-filter-label">Seller</label>
            <select
            ...
            >
              ...
              {sellers
                .filter(s => s.sellerTradeName || s.sellerName)
                .sort((a, b) => (a.sellerTradeName || a.sellerName).localeCompare(b.sellerTradeName || b.sellerName))
                .map((s) => (
                  <option key={s._id} value={s.sellerTradeName || s.sellerName}>
          ...
          <thead>
            <tr>
              <th onClick={() => requestSort('companyName')} style={{ cursor: 'pointer' }}>
                SELLER COMPANY NAME {sortConfig.key === 'companyName' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}
              </th>
              {isSuperAdmin && (
                <th onClick={() => requestSort('buyerTradeName')} style={{ cursor: 'pointer' }}>
                  COMPANY NAME {sortConfig.key === 'buyerTradeName' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}
                </th>
              )}
              <th onClick={() => requestSort('companyState')} style={{ cursor: 'pointer' }}>
                PLACE OF SUPPLY {sortConfig.key === 'companyState' ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}
              </th>
              ...
              {hasPermission('Purchase Order', 'view') && <th>PRINT</th>}
              {(hasPermission('Purchase Order', 'update') || hasPermission('Purchase Order', 'delete')) && <th>ACTIONS</th>}
            </tr>
          </thead>
          <tbody>
            {pagedRows.length === 0 ? (
              ...
              pagedRows.map((r) => (
                <tr key={r._id}>
                  <td
                    ...
                    onClick={() => handleSellerClick(r)}
                  >
                    {r.companyName}
                  </td>
                  {isSuperAdmin && <td>{r.buyerTradeName}</td>}
                  <td>{r.companyState}</td>
                  <td>{r.invoiceNo}</td>
                  <td>{formatDateDDMMYYYY(r.invoiceDate)}</td>
                  <td>{(parseFloat(r.totalAmount) * 1.18 || 0).toFixed(2)}</td>
                  {hasPermission('Purchase Order', 'view') && (
                    <td>
                      <button type="button" className="list-page-icon-btn" title="Preview" onClick={() => onPreview && onPreview(r)}>
                  ...
                  {(hasPermission('Purchase Order', 'update') || hasPermission('Purchase Order', 'delete')) && (
                    <td>
                      <div className="list-page-action-icons">
                        {hasPermission('Purchase Order', 'update') && (
                          <button type="button" className="list-page-icon-btn" title="Edit" aria-label="Edit" onClick={() => startEdit(r)}>
      ...
      <div className="list-page-footer">
        ...
        <div className="list-page-info">Showing {showingFrom} to {showingTo} of {total} entries</div>
      ...
      <AddPurchaseOrderForm isOpen={showAddForm} onClose={() => { closeForm(); fetchPurchaseInvoices(); }} editingRow={editingRow} />
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
              {Object.entries(columnLabels).map(([key, label]) => {
                if (key === 'buyerTradeName' && !isSuperAdmin) return null;
                return (
