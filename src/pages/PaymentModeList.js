...
function PaymentModeList({ onAdd, onEdit }) {
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            fetchPaymentModes(searchQuery, currentPage, itemsPerPage);
        }, 200);
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await fetchApi(`/payment-mode/${id}`, { method: 'DELETE' });
                    setData(prevData => prevData.filter(item => item._id !== id));
                    Swal.fire('Deleted!', 'Payment mode has been deleted.', 'success');
                } catch (error) {
    ...
    // if (loading) return <div className="list-page-container">Loading payment modes...</div>;


    return (
        <div className="list-page-container">
            <div className="list-page-header">
                <h1 className="list-page-title">Payment Mode List</h1>
                <div className="list-page-action-buttons">
                    {hasPermission('PaymentMode', 'add') && (
                        <button className="list-page-btn list-page-btn-add" onClick={handleAdd}>ADD PAYMENT MODE</button>
                    )}
                </div>
            </div>
            ...
            <div className="list-page-table-wrapper">
                <table className="list-page-table">
                    <thead><tr><th>PAYMENT MODE</th><th>ACTIONS</th></tr></thead>
                    <tbody>
                        {loading ? (
                            <tr><td className="list-page-table-empty" colSpan={2}>Loading...</td></tr>
                                ...
                                <tr key={item._id}>
                                    ...
                                    <td>
                                        <div className="list-page-action-icons">
                                            {hasPermission('PaymentMode', 'update') && (
                                            ...
                                            {hasPermission('PaymentMode', 'delete') && (
