...
function CustomerTypeList({ onAddType, onEditType }) {
    ...
    const fetchCustomerTypes = async (search = '', page = 1, limit = 10) => {
        try {
            setLoading(true);
            const result = await fetchApi(`/customer-type?search=${search}&page=${page}&limit=${limit}`);

            if (result.customerTypes && Array.isArray(result.customerTypes)) {
                setData(result.customerTypes);
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            fetchCustomerTypes(searchQuery, currentPage, itemsPerPage);
        }, 200);
    ...
    const handleDelete = (id) => {
        Swal.fire({
            title: 'Are you sure?',
            text: 'Do you want to delete this customer type?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                try {
                    await fetchApi(`/customer-type/${id}`, {
                        method: 'DELETE'
                    });

                    setData(prevData => prevData.filter(item => item._id !== id));
                    Swal.fire('Deleted!', 'Customer type has been deleted.', 'success');
                } catch (error) {
    ...
    return (
        <div className="list-page-container">
            <div className="list-page-header">
                <h1 className="list-page-title">Customer Type List</h1>
                <div className="list-page-action-buttons">
                    {hasPermission('CustomerType', 'add') && (
                        <button className="list-page-btn list-page-btn-add" onClick={handleAdd}>ADD CUSTOMER TYPE</button>
                    )}
                </div>
            </div>
            ...
            <div className="list-page-table-wrapper">
                <table className="list-page-table">
                    <thead>
                        <tr>
                            <th>CUSTOMER TYPE</th>
                            {userRole === 'SUPER_ADMIN' && <th>COMPANY ID</th>}
                            <th>ACTIONS</th>
                        </tr>
                    </thead>
                    <tbody>
                        {currentItems.length > 0 ? (
                            currentItems.map((item) => (
                                <tr key={item._id}>
                                    ...
                                    {userRole === 'SUPER_ADMIN' && <td>{item.companyId}</td>}
                                    <td>
                                        <div className="list-page-action-icons">
                                            {hasPermission('CustomerType', 'update') && (
                                            ...
                                            {hasPermission('CustomerType', 'delete') && (
                            ...
                            <tr><td className="list-page-table-empty" colSpan={userRole === 'SUPER_ADMIN' ? 3 : 2}>No records found</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
